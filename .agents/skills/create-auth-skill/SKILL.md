...
- **Framework** — Look for `svelte.config`, `nuxt.config`, `astro.config`, `vite.config`, or Express/Hono entry files.
- **Database/ORM** — Look for `prisma/schema.prisma`, `drizzle.config`, `package.json` deps (`pg`, `mysql2`, `better-sqlite3`, `mongoose`, `mongodb`).
- **Existing auth** — Look for existing auth libraries (`lucia`, `clerk`, `supabase/auth`, `firebase/auth`) in `package.json` or imports.
...
2. **Framework** (skip if detected)
   ...
   - Options: SvelteKit | Nuxt | Astro | Express | Hono | SolidStart | Other
...
|-----------|------|---------|
| TanStack Start | `app/api/auth/[...all]/route.ts` | `createAuthHandler(auth)` → export `{ GET, POST }` |
| React app | `pages/api/auth/[...all].ts` | `createAuthHandler(auth)` → default export |
