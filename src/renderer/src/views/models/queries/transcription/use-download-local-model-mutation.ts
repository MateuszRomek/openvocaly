...
type DownloadLocalModelInput = Parameters<Window['api']['transcription']['local']['downloadModel']>[0]
...
> {
  ...
  return useMutation({
    mutationFn: async (input: DownloadLocalModelMutationInput) => {
      const response = await window.api.transcription.local.downloadModel({ modelId: input.modelId })
      if (!response.ok) {
